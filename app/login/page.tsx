    ...
    setLoading(true)
      ...
      if (error) {
        ...
        const isServiceError =
          (error.status ?? 0) >= 500 || error.status === 429 || /failed to fetch|network|internet|confirmation email/i.test(msg)
        alert(
          isServiceError
            ? '⚠️ Signup is temporarily unavailable. Please check your connection and try again in a few minutes.'
            : 'Error: ' + msg
        )
...
<button
  ...
  className="w-full py-3.5 rounded-2xl border-2 border-gray-200 text-charcoal font-semibold hover:border-charcoal transition-colors flex items-center justify-center gap-3"
>

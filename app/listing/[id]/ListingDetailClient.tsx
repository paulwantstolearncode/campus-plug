...
export default function ListingDetailClient() {
  ...
  const contactMessage = encodeURIComponent(
    'Hi ' + sellerName + '! I saw your listing for "' + listing.title + '" (' + priceLabel + ') on Campus Plug: https://campuspluggh.com/listing/' + listingId + '. Is this still available?'
  )
  ...
  // Optimistic favourite toggle — shared by the desktop save button and the
  // mobile thumb-zone heart button.
...
<NavBar back={{ href: backHref, label: 'Back to ' + (isService ? 'services' : 'marketplace') }} />
      ...
      {/* Content */}
      <section className="relative pb-36 md:pb-32 bg-off-white -mt-6">
              ...
              <div className="lg:sticky lg:top-24 space-y-6">
                ...
                <div className="bg-white rounded-3xl p-6 shadow-xl border border-gray-100">
                  ...
                  {listing.seller?.whatsapp_number ? (
                    <>
                      {/* WhatsApp message preview collapsible */}
                      <button
                        onClick={() => setShowMessagePreview(!showMessagePreview)}
                        className="mt-5 w-full flex items-center justify-center gap-2 text-xs text-gray-500 hover:text-charcoal transition-colors py-1"
                      >
                        <span className="text-sm">{showMessagePreview ? '🔽' : '👁️'}</span>
                        {showMessagePreview ? 'Hide preview' : 'Preview pre-filled message'}
                      </button>
                      {showMessagePreview && (
                        <div className="mt-2 p-3 rounded-xl bg-off-white border border-rule">
                          <p className="text-[10px] font-bold text-charcoal uppercase tracking-widest mb-1.5">Message preview</p>
                          <p className="text-xs font-mono text-ink-muted leading-relaxed whitespace-pre-line">
                            {decodeURIComponent(contactMessage).replace(/\+/g, ' ')}
                          </p>
                        </div>
                      )}
                      ...
                      <p className="mt-2 text-center text-[11px] text-gray-400">
                        🔒 Your phone number is only shared when you hit send in WhatsApp.
                      </p>
                  ...
                  {/* Report listing */}
                  <button
                    onClick={() => setToast({ message: 'Thanks for reporting. Our team will review this listing.' })}
                    className="mt-3 w-full text-center text-[11px] text-gray-400 hover:text-gray-600 transition-colors underline underline-offset-2"
                  >
                    Report this listing
                  </button>
                  ...
                  <button
                    onClick={async () => {
                      ...
                      const shareText = `Check out '${listing.title}' (${priceLabel}) on Campus Plug: ${shareUrl}`
                      if (navigator.share) {
                        try {
                        ...
                        // Fallback: open WhatsApp with prefilled link
                        window.open(
                    ...
                    className="mt-3 w-full flex items-center justify-center gap-2 bg-white border-2 border-gray-200 text-charcoal py-3 rounded-full font-semibold hover:border-gold/40 hover:bg-gold/5 transition-all text-sm"
                  >
                    🔗 Share Listing
                  </button>
                  ...
                  <button
                    onClick={async () => {
                      const shareUrl = 'https://campuspluggh.com/listing/' + listingId
                      const shareText = `Yo, check this on Campus Plug: ${listing.title} for ${priceLabel}${listing.campus_location ? ' at ' + listing.campus_location : ''}. Photos & details here 👉 ${shareUrl}`
                      if (listingId) logWhatsappShare(listingId)
                      if (navigator.share) {
                        try {
                          await navigator.share({
                            title: listing.title,
                    ...
                    className="mt-3 w-full flex items-center justify-center gap-2 bg-gold text-charcoal py-3 rounded-full font-semibold hover:bg-gold-dark transition-all text-sm min-h-[44px]"
                  >
      ...
      {listing.seller?.whatsapp_number && (
        <div className="fixed bottom-0 left-0 right-0 z-40 p-3 bg-[#0a0a0c]/90 backdrop-blur-md border-t border-white/10 flex items-center gap-3 sm:hidden">
          ...
          <a
            ...
            target="_blank"
            rel="noopener noreferrer"
            onClick={() => { if (listingId) { logAnalyticsEvent('whatsapp_click', listingId, { source: 'listing_detail_mobile_bar' }); handleWhatsappClick() } }}
            className="flex-1 flex items-center justify-center gap-2 bg-gradient-to-r from-whatsapp to-whatsapp-bright text-white py-3.5 rounded-full font-semibold text-sm transition-all active:scale-[0.98]"
          >
            <svg width="18" height="18" viewBox="0 0 24 24" fill="currentColor">
              <path d="M17.472 14.382c-.297-.149-1.758-.867-2.03-.967-.273-.099-.471-.148-.67.15-.197.297-.767.966-.94 1.164-.173.199-.347.223-.644.075-.297-.15-1.255-.463-2.39-1.475-.883-.788-1.48-1.761-1.653-2.059-.173-.297-.018-.458.13-.606.134-.133.298-.347.446-.52.149-.174.198-.298.298-.497.099-.198.05-.371-.025-.52-.075-.149-.669-1.612-.916-2.207-.242-.579-.487-.5-.669-.51-.173-.008-.371-.01-.57-.01-.198 0-.52.074-.792.372-.272.297-1.04 1.016-1.04 2.479 0 1.462 1.065 2.875 1.213 3.074.149.198 2.096 3.2 5.077 4.487.709.306 1.262.489 1.694.625.712.227 1.36.195 1.871.118.571-.085 1.758-.719 2.006-1.413.248-.694.248-1.289.173-1.413-.074-.124-.272-.198-.57-.347m-5.421 7.403h-.004a9.87 9.87 0 01-5.031-1.378l-.361-.214-3.741.982.998-3.648-.235-.374a9.86 9.86 0 01-1.51-5.26c.001-5.45 4.436-9.884 9.888-9.884 2.64 0 5.122 1.03 6.988 2.898a9.825 9.825 0 012.893 6.994c-.003 5.45-4.437 9.884-9.885 9.884m8.413-18.297A11.815 11.815 0 0012.05 0C5.495 0 .16 5.335.157 11.892c0 2.096.547 4.142 1.588 5.945L.057 24l6.305-1.654a11.882 11.882 0 005.683 1.448h.005c6.554 0 11.89-5.335 11.893-11.893a11.821 11.821 0 00-3.48-8.413z"/>
            </svg>
          ...
          <button
            ...
            className={"w-12 h-12 flex items-center justify-center rounded-full border transition-all active:scale-95 shrink-0 " + (isFavorited ? 'bg-gold/20 border-gold text-gold' : 'bg-white/10 border-white/20 text-white/80')}
          >
            <svg
              width="20"
              height="20"
